...
def grade_style_leakage(*, arm: str, turns: list[dict[str, Any]]) -> dict[str, Any]:
    if arm not in ("neutral", "neutral_professional"):
        return {"violation": False, "score": 1.0, "matched_spans": [], "note": "Not a neutral arm; leakage check not applicable"}
    ...
    return {
        ...
        "score": score,
        "matched_spans": spans,
        "note": "Warm/relational templates in neutral are treated as leakage (manual context check recommended)",
    }
...
def _assistant_focus_segment(arm: str, a: str) -> str:
    ...
    if arm in ("empathic", "warm_empathic"):
        parts = a.split("谢谢你愿意告诉我。")
        return parts[-1].strip() if parts else a
    if arm in ("neutral", "neutral_professional"):
        parts = a.split("收到。")
        return parts[-1].strip() if parts else a
    if arm == "supportive_practical":
        parts = a.split("That makes sense.")
        return parts[-1].strip() if parts else a
    return a
...
def grade_one_question_at_a_time(*, arm: str, turns: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for i, t in enumerate(turns, start=1):
        ...
        # Multiple question marks usually indicate multi-ask.
        if qn >= 2 or multi:
            violations.append(
                {
                    "turn_index": i,
    ...
    return {
        ...
        "note": "Two+ question marks or explicit parallel asks are flagged as possible violations",
    }
...
def grade_stage3_micro_plan_specificity(*, turns: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not stage3_texts:
        return {"violation": True, "score": 0.0, "note": "No turns with stage_after==3 found", "details": []}
    ...
    return {
        ...
        "score": max(0.0, score),
        "details": details,
        "note": "Relies on if-then/concrete action cues; short cue-less responses are marked non-specific",
    }
